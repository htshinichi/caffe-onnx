...
def getConvOutShape(input_shape,layer,dict):
    ...
    #计算输入维度output_shape
    h = (input_shape[0][2] - kernel_shape[0] + 2 * pads[0] - (kernel_shape[0]-1)*(dilations[0]-1))/strides[0] + 1 # 输出维度N= ((输入维度I - 卷积核维度K + 2 * 填充P - (卷积核维度-1)*(膨胀系数-1))/步长S) + 1
    ...
    w = (input_shape[0][3] - kernel_shape[1] + 2 * pads[1] - (kernel_shape[1]-1)*(dilations[1]-1))/strides[1] + 1 # 输出维度N= ((输入维度I - 卷积核维度K + 2 * 填充P - (卷积核维度-1)*(膨胀系数-1))/步长S) + 1
    #当h非整数 ,且未设置pad ,在遇到输出为非整数情况 ,向上取整 ,即在右边和下边补1
